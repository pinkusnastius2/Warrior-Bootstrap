...
// Wrapper function for is_writable() for Windows compatibility
  ...
  /* rewritten for BB-Gunshop products specification and products History*/
  function tep_get_products_history($product_id, $language_id) {
    $product_query = tep_db_query("select products_history from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_history'];
  }
  function tep_get_products_spec($product_id, $language_id) {
   ...
   function tep_get_products_image_spec($product_id, $language_id) {
    $product_query = tep_db_query("select products_image_spec from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_image_spec'];
  }
    function tep_get_products_links($product_id, $language_id) {
    $product_query = tep_db_query("select products_links from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_links'];
  }
  /*End BB gunshop rewrite*/
?>