    ...
    switch ($action) {
      ...
      case 'update_category':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		  ...
		  $sql_desc_array= array('categories_description' => tep_db_prepare_input($categories_description_array[$language_id]));
          ...
          if ($action == 'insert_category') {
			...
			tep_db_perform(TABLE_CATEGORIES_DESCRIPTION, $sql_desc_array);
          } elseif ($action == 'update_category') {
            tep_db_perform(TABLE_CATEGORIES_DESCRIPTION, $sql_data_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
			tep_db_perform(TABLE_CATEGORIES_DESCRIPTION, $sql_desc_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
          }
        }
...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        ...
        //if ($products_image->parse() && $products_image->save()) {
          $sql_data_array['products_image'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image'],7));
		  $sql_data_array['products_image_sm_1'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image_sm_1'],7));
		  $sql_data_array['products_image_sm_2'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image_sm_2'],7));
		  $sql_data_array['products_image_sm_3'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image_sm_3'],7));
		  $sql_data_array['products_image_sm_4'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image_sm_4'],7));
...
/** AJAX Attribute Manager  end **/
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          $sql_data_array = array('products_name' => tep_db_prepare_input($HTTP_POST_VARS['products_name'][$language_id]),
                                  ...
                                  'products_image_spec' => tep_db_prepare_input($HTTP_POST_VARS['products_image_spec'][$language_id]),
								  'products_links' => tep_db_prepare_input($HTTP_POST_VARS['products_links'][$language_id]),
                                  'products_history' => tep_db_prepare_input($HTTP_POST_VARS['products_history'][$language_id]),
...
// Update existing large product images
	...
	/*		if(preg_match('/^products_image_large_new_([0-9]+)$/', $key, $matches)) {
              $sql_data_array['image'] = tep_db_prepare_input(substr($HTTP_POST_VARS['products_image_large_new' . $matches[1]],7));
			 }
...
// check if the catalog image directory exists
  ...
  require(DIR_WS_INCLUDES . 'template_top_cat.php');
  ...
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_image_sm_1, p.products_image_sm_2,  p.products_image_sm_3,  p.products_image_sm_4,date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
...
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0)?>Products Specification</td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('products_spec[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (empty($pInfo->products_id) ? '' : tep_get_products_spec($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr> 
            </table></td>
          </tr>
<?php
...
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0)?>Products Image Specification</td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('products_image_spec[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (empty($pInfo->products_id) ? '' : tep_get_products_image_spec($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr> 
            </table></td>
          </tr>
<?php
...
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0)?>Products Links</td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('products_links[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (empty($pInfo->products_id) ? '' : tep_get_products_links($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr> 
            </table></td>
          </tr>
<?php
...
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0)?>Products History</td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('products_history[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (empty($pInfo->products_id) ? '' : tep_get_products_history($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr> 
            </table></td>
          </tr>
<?php
...
?>
          ...
          <tr>
            ...
            <td class="main" style="padding-left: 30px;">
              <div><?php echo '<strong>' . TEXT_PRODUCTS_MAIN_IMAGE . ' <small>(' . SMALL_IMAGE_WIDTH . ' x ' . SMALL_IMAGE_HEIGHT . 'px)</small></strong><br />' . (tep_not_null($pInfo->products_image) ? '<a href="' . DIR_WS_CATALOG_IMAGES . $pInfo->products_image . '" target="_blank">' . $pInfo->products_image . '</a> &#124; ' : '') . tep_draw_input_field('products_image','','class="kfm" id="products_image" value="/images' . $pInfo->products_image .'"'); ?></div>
              <div><?php echo '<strong>' . TEXT_PRODUCTS_MAIN_IMAGE . '2 <small>(' . SMALL_IMAGE_WIDTH . ' x ' . SMALL_IMAGE_HEIGHT . 'px)</small></strong><br />' . (tep_not_null($pInfo->products_image_sm_1) ? '<a href="' . DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_1 . '" target="_blank">' . $pInfo->products_image_sm_1 . '</a> &#124; ' : '') . tep_draw_input_field('products_image_sm_1','','class="kfm" id="products_image_sm_1" value="/images' . $pInfo->products_image_sm_1 .'"'); ?></div>
<div><?php echo '<strong>' . TEXT_PRODUCTS_MAIN_IMAGE . '3 <small>(' . SMALL_IMAGE_WIDTH . ' x ' . SMALL_IMAGE_HEIGHT . 'px)</small></strong><br />' . (tep_not_null($pInfo->products_image_sm_2) ? '<a href="' . DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_2 . '" target="_blank">' . $pInfo->products_image_sm_2 . '</a> &#124; ' : '') . tep_draw_input_field('products_image_sm_2','','class="kfm" id="products_image_sm_2" value="/images' . $pInfo->products_image_sm_2 .'"'); ?></div>
<div><?php echo '<strong>' . TEXT_PRODUCTS_MAIN_IMAGE . '4 <small>(' . SMALL_IMAGE_WIDTH . ' x ' . SMALL_IMAGE_HEIGHT . 'px)</small></strong><br />' . (tep_not_null($pInfo->products_image_sm_3) ? '<a href="' . DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_3 . '" target="_blank">' . $pInfo->products_image_sm_3 . '</a> &#124; ' : '') . tep_draw_input_field('products_image_sm_3','','class="kfm" id="products_image_sm_3" value="/images' . $pInfo->products_image_sm_3 .'"'); ?></div>
<div><?php echo '<strong>' . TEXT_PRODUCTS_MAIN_IMAGE . '5 <small>(' . SMALL_IMAGE_WIDTH . ' x ' . SMALL_IMAGE_HEIGHT . 'px)</small></strong><br />' . (tep_not_null($pInfo->products_image_sm_4) ? '<a href="' . DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_4 . '" target="_blank">' . $pInfo->products_image_sm_4 . '</a> &#124; ' : '') . tep_draw_input_field('products_image_sm_4','','class="kfm" id="products_image_sm_4" value="/images' . $pInfo->products_image_sm_4 .'"'); ?></div>
  ...
  var els=document.getElementsByTagName('*');
	...
	for(i in els){
		...
		if(reg.test(el.className))el.onclick=function(){
			...
			window.open(kfm_url,'kfm','modal,width=800,height=1000');
		}
	}
}
   ...
   if (HTML_AREA_WYSIWYG_DISABLE == 'Disable') {} else { ?>
          ...
          <?php } ?>
            ...
            <script language="JavaScript1.2" defer>
             ...
             editor_generate('products_image_1',additonal_image);
             editor_generate('products_image_2',additonal_image);
             editor_generate('products_image_3',additonal_image);
             editor_generate('products_image_4',additonal_image);
             editor_generate('products_image_5',additonal_image);
             editor_generate('products_image_6',additonal_image);
             editor_generate('products_image_7',additonal_image);
             editor_generate('products_image_8',additonal_image);
             editor_generate('products_image_9',additonal_image);
             editor_generate('products_image_10',additonal_image);
             editor_generate('products_image_11',additonal_image);
             editor_generate('products_image_12',additonal_image);
			 editor_generate('info_logo',additonal_image);
			 editor_generate('products_data',addition_image);
            </script>
<?php } */?>
<?php
...
for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          $category_description_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_textarea_field('categories_description[' . $languages[$i]['id'] . ']','soft', '70','15', tep_get_category_description($cInfo->categories_id, $languages[$i]['id']));
        }

        $contents[] = array('text' => '<br>' . 'Edit Category Description' . $category_description_string);
