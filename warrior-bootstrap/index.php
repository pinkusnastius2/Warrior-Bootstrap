...
// check to see if there are deeper categories within the current category
      ...
      echo '<h4>' . tep_get_category_description((int)$category_links[$i], (int)$languages_id) .'</h4>';
	      break; // we've found the deepest category the customer is in
        }
      }
    } else {
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . (int)$current_category_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by sort_order, cd.categories_name");
	        echo '<h4>' . tep_get_category_description((int)$current_category_id, (int)$languages_id) .'</h4>';
    }
...
// We are asked to show only a specific category
        $listing_sql = "select " . $select_column_list . " p.products_id, pd.products_image_spec, SUBSTRING_INDEX(pd.products_description, ' ', 90) as products_description, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_status = '1' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "' and p.products_id = p2c.products_id and pd.products_id = p2c.products_id and pd.language_id = '" . (int)$languages_id . "' and p2c.categories_id = '" . (int)$HTTP_GET_VARS['filter_id'] . "'";
      } else {
// We show them all
...
<?php   $fr_query=tep_db_query("select * from front_page_image ORDER BY RAND() LIMIT 6 ");
	$firstpanel=0;
	while($ads=tep_db_fetch_array($fr_query)){
		
		echo '<div class="item';
		if($firstpanel==0){ echo ' active'; $firstpanel++;}
		echo '"><a href="' . $ads['FP_Link'] .'">' . tep_image($ads['FP_Image']) .'</a>  <div class="carousel-caption">
      </div></div>';//	<div id="title" style="padding: 10px 15px; position: absolute; top: 20px; right: 30px; z-index: 3; border-bottom: 1px solid rgba(100,210,90,1);border-right: 1px solid rgba(100,210,90,1);border-top: 1px solid #d341d8;border-left: 1px solid #d341d8; font-family:Verdana, Geneva, sans-serif; font-weight:bold; background: rgba(3,3,3,0.7); color: #d341d8; -webkit-border-radius: 10px; -moz-border-radius: 10px; border-radius: 10px">' . $ads['FP_Title'] . '</div>';
	}?>
